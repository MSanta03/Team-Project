...
public class ControllerAdminHome {
	...
	*/
	...
	protected static boolean invalidEmailAddress(String emailAddress) {
		// TP1-14 / TP1-15: validate the value before it is used for a database lookup
		// or invitation. InputValidator checks the maximum size first and then checks
		// that the required email has a reasonable email format.
		String validationError = InputValidator.validateEmailAddress(emailAddress);
		if (!validationError.isEmpty()) {
			ViewAdminHome.alertEmailError.setContentText(validationError);
			ViewAdminHome.alertEmailError.showAndWait();
			return true;
		}
		return false;
