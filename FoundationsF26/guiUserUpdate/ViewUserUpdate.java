...
public class ViewUserUpdate {
	...
	private static Alert alertValidationError = new Alert(AlertType.ERROR);
	...
	private ViewUserUpdate() {
		...
		alertValidationError.setTitle("Invalid Input");
		alertValidationError.setHeaderText("Please correct the value and try again.");
        ...
        button_UpdateFirstName.setOnAction((_) -> {
            ...
            result.ifPresent(input -> updateNameField(input, "First name", 1));
        });
        ...
        button_UpdateMiddleName.setOnAction((_) -> {
            ...
            result.ifPresent(input -> updateNameField(input, "Middle name", 2));
        });
        ...
        button_UpdateLastName.setOnAction((_) -> {
            ...
            result.ifPresent(input -> updateNameField(input, "Last name", 3));
        });
        ...
        button_UpdatePreferredFirstName.setOnAction((_) -> {
            ...
            result.ifPresent(input -> updateNameField(input, "Preferred first name", 4));
        });
        ...
        button_UpdateEmailAddress.setOnAction((_) -> {
            ...
            result.ifPresent(input -> {
                // TP1-14 / TP1-15: validate before sending the email to the database.
                String validationError = InputValidator.validateEmailAddress(input);
	...
	/**
	 * Validates and updates one of the personal-name fields.  TP1 requires a defensive maximum
	 * size check, but does not restrict names to letters because valid names may contain spaces,
	 * hyphens, apostrophes, accents, and other characters.
	 *
	 * @param input value entered in the update dialog
	 * @param fieldName field name used in an error message
	 * @param fieldNumber identifies which personal-name database field is being updated
	 */
	private static void updateNameField(String input, String fieldName, int fieldNumber) {
		String validationError = InputValidator.validateMaximumLength(input, fieldName);
		if (!validationError.isEmpty()) {
			showValidationError(validationError);
			return;
		}
		...
		case 1:
			...
			theUser.setFirstName(value);
			label_CurrentFirstName.setText(value.isEmpty() ? "<none>" : value);
			break;
		case 2:
			theDatabase.updateMiddleName(theUser.getUserName(), value);
			theUser.setMiddleName(value);
			label_CurrentMiddleName.setText(value.isEmpty() ? "<none>" : value);
			break;
		case 3:
			theDatabase.updateLastName(theUser.getUserName(), value);
			theUser.setLastName(value);
			label_CurrentLastName.setText(value.isEmpty() ? "<none>" : value);
			break;
		case 4:
			theDatabase.updatePreferredFirstName(theUser.getUserName(), value);
			theUser.setPreferredFirstName(value);
			label_CurrentPreferredFirstName.setText(value.isEmpty() ? "<none>" : value);
			break;
		default:
			break;
		}
	...
	/** Display a validation error without changing the current database value. */
	private static void showValidationError(String message) {
		alertValidationError.setContentText(message);
