...
public class ControllerUserLogin {
	...
	*/
	...
	protected static void doLogin(Stage ts) {
		...
		// check to make sure the username is valid, if not alert the user and wait for them to
		// fix the error
		if (usernameErr != "") {
			ViewUserLogin.alertUsernamePasswordError.setContentText(usernameErr);
