	...
	private ViewUserUpdate() {
        ...
        button_UpdateFirstName.setOnAction((_) -> {
        	...
        	if (result.isPresent()) {
        		...
        		// Validate the name before it is used to update the database.
        		if (newName.length() > 255) {
        			Alert alertInputError = new Alert(AlertType.INFORMATION);
        			alertInputError.setTitle("Invalid Input");
        			alertInputError.setHeaderText(null);
        			alertInputError.setContentText(
        					"This field may not exceed 255 characters.");
        ...
        button_UpdateEmailAddress.setOnAction((_) -> {
        	...
        	if (result.isPresent()) {
        		...
        		String emailEvaluation = EmailAddressRecognizer.checkEmailAddress(newEmail);
        		
        		// Validate the email address before updating the database.
        		if (!emailEvaluation.equals("")) {
        			Alert alertEmailError = new Alert(AlertType.INFORMATION);
        			alertEmailError.setTitle("Invalid Email Address");
        			alertEmailError.setHeaderText("The email address is not valid.");
        			alertEmailError.setContentText(emailEvaluation);
        		...
        		theDatabase.getUserAccountDetails(theUser.getUserName());
        		newEmail = theDatabase.getCurrentEmailAddress();
