...
public class ControllerUserLogin {
	...
	*/
	...
	protected static void doLogin(Stage ts) {
    	...
    	// Validate the login input before it is used for a database lookup.
    	...
    	if (username.length() > 255 || password.length() > 255) {
    		ViewUserLogin.alertUsernamePasswordError.setContentText(
    				"Username and password inputs may not exceed 255 characters.");
    		ViewUserLogin.alertUsernamePasswordError.showAndWait();
    		return;
    	}
        ...
        // Check to see if the password entered is the users active one time password
     	if (theDatabase.isValidOneTimePassword(username, password)) {
     		...
     		System.out.println("One-Time Password Used!");
	...
	protected static void doSetupAccount(Stage theStage, String invitationCode) {
		...
		// Validate the invitation code before it is used for a database lookup.
		if (invitationCode == null || invitationCode.length() == 0) {
			ViewUserLogin.alertUsernamePasswordError.setTitle("Invalid Invitation Code");
			ViewUserLogin.alertUsernamePasswordError.setContentText(
					"Enter an invitation code and try again.");
			ViewUserLogin.alertUsernamePasswordError.showAndWait();
			return;
		}
		
		if (invitationCode.length() > 255) {
			ViewUserLogin.alertUsernamePasswordError.setTitle("Invalid Invitation Code");
			ViewUserLogin.alertUsernamePasswordError.setContentText(
					"The invitation code may not exceed 255 characters.");
			ViewUserLogin.alertUsernamePasswordError.showAndWait();
			return;
		}
